...
export class PageComponent implements OnInit {
  ...
  influencer_link: string = window.location.href;
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((params) => {
      this.influencer_url = params.get('id') || '';
    });
    this.fetchData();
  }

  fetchData() {
    this.influencerService.getByUrl(this.influencer_url).subscribe({
      next: (response: HttpResponse<Influencer>) => {
        ...
        this.influencer.image = response.body?.image || '';
        this.entityForm.get('influencer')?.setValue(this.influencer.id);
        this.campaignService
          .listInfluencerCampaignsActive(this.influencer.id)
          .subscribe({
  ...
  private initializeProfileForm() {
    this.entityForm = this.formBuilder.group({
      ...
      dissemination_mode: new FormControl('INFLUENCER', Validators.required),
      detection_type: new FormControl('TYPED', Validators.required),
      influencer: new FormControl('', Validators.required),
