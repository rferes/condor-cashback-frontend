...
import { Influencer } from '../../profile/entities/profile.entity';
import { HttpResponse, HttpErrorResponse } from '@angular/common/http';
import { DashboardInfluencer } from '../entities/dashboard.entity';
...
export class DashboardComponent implements OnInit {
  ...
  private endURL: string = 'receipts/receipt_campaign_csv/';
  private endURLMensal: string = 'receipts/receipt_csv_mensal/';
  profile: Influencer = {} as Influencer; // Initialize profile
  dashboardData: DashboardInfluencer = {} as DashboardInfluencer;
  ...
  downloadCSV() {
    const fileName: string = `RedRed_Notas_Geral_Influencer_${this.profile.name}.csv`;
  ...
  downloadCSVmensal() {
    const fileName: string = `RedRed_Relatorio_Mensal_Influencer_${this.profile.name}_${this.csvMensalForm.value.month_code}.zip`;
    const formData = new FormData();
    formData.append('profile_id', this.profile.id.toString());
    ...
    this.profileService.get('me').subscribe({
      next: (response: HttpResponse<Influencer>) => {
        ...
        this.profileService.dashboard_data(String(this.profile.id)).subscribe({
          next: (response: HttpResponse<DashboardInfluencer>) => {
