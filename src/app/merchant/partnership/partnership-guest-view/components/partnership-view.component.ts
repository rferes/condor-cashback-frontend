...
interface MerchantPartnerGuest {
  id: string;
  name: string;
  image: string;
  document: string;
...
@Component({
  selector: 'app-partnership-guest-view',
  templateUrl: '../views/partnership-view.component.html',
})
export class PartnershipGuestViewComponent implements OnInit {
  isFavorited = false;
  merchantPartnerGuest = {} as MerchantPartnerGuest;
  constructor(
  ...
  fetchMerchant(pk: string) {
    this.partnershipService.get_partnership_guest_data(pk).subscribe({
      next: (res) => {
        if (res.body) {
          this.merchantPartnerGuest = {
            id: res.body.id,
            name: res.body.name,
